...
function create_shortcode_tool_calorie($args, $content) {
			...
			<form action="" id="clorieToolForm">
				<div class="form-row">
					<div class="form-col">
						...
						<div class="form-radio-list gender-radio">
				...
				<div class="form-row">
					...
					<?php if(wp_is_mobile()) { ?>
					<div class="calories-sp">
						<label for="">Level of Activity</label>
						<select class="select-ip input-it" name="calorie_level">
				...
				<div class="form-row">
					...
					<div class="form-col">
						...
						<?php } ?>
						<input type="submit" value="Calculate" class="calories-submit">
					</div>
				</div>
			</form>
		</div>
	...
	wp_enqueue_script( 'calorie-js', get_template_directory_uri() . '/shortcode/calorie/assets/js/calorie-tool.js','','1.0.0');
...
function get_calorie_tool() {
	if(isset($_GET['get_calorie_tool']) && is_ajax_calorie_tool()){
		...
		if($calorie_unit == 1) $unit_label = 'Calories';
		else $unit_label = 'Kilojoules';
		...
		<div class="calories-result">
			...
			<div class="calories-col">
				<ul>
					...
					?>
					<li>
						<p><?php echo $rc->name; ?> <small><?php echo $rc->description; ?></small></p>
						<p><b><?php echo number_format($rc->calorie); ?></b><span>100%</span><small><?php echo $unit_label; ?>/day</small></p>
					</li>
					<?php } } 
					if($result_cal->gain) {
				...
				<?php if($result_cal->gain) { ?><p><a href="#" class="show-wgain">Show info for weight gain</a></p><?php } ?>
			</div>
		</div>
		<?php if($zigzag_schedule_1 || $zigzag_schedule_2) { ?>
		<h3>Zigzag Calorie Cycling:</h3>
		<p>As you keep a low-calorie diet, your body will likely adapt to the new, lower energy environment, which can lead to a plateau in your progress. </p>
		<p>Zigzag calorie cycling, also known as a "zigzag diet," is a method of calorie consumption that can potentially help you overcome this plateau and get you back on track to meeting your goals. <a href="#zigzag">Click here</a> to learn more about zigzag diet. The following are two sample 7-day Zigzag calorie cycling schedules.</p>
		...
		<figure class="wp-block-table">
			<table>
				<tr>
					...
					<?php if($mild_weight) { ?><td>Mild weight loss</td><?php } ?>
					<?php if($weight_loss) { ?><td>Weight loss</td><?php } ?>
					<?php if($extreme_loss) { ?><td>Extreme weight loss</td><?php } ?>
				</tr>
				<?php if($mild_weight) { 
				...
				<tr>
					...
					<?php if($weight_loss) { ?><td><?php echo number_format($weight_loss[$m]->calorie); ?> <?php echo $unit_label; ?></td><?php } ?>
					<?php if($extreme_loss) { ?><td><?php echo number_format($extreme_loss[$m]->calorie); ?> <?php echo $unit_label; ?></td><?php } ?>
				</tr>
				<?php } } ?>
			</table>
		</figure>
		...
		<p>Another effective way to lose weight, aside from reducing calorie intake, is increasing your activity level. The following is a general list of estimated weight lost based on varying activity levels and the maintenance intake of 2,549 calories per day.</p>
		<figure class="wp-block-table">
			<table>
				<tr>
					<td>Activity level</td>
