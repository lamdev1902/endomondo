...
function ideal_weight_call_shortcode($info) {
	...
	curl_setopt_array($curl, array(
	  CURLOPT_URL => 'https://dev.ehproject.org/wp-json/api/v1/ideal-weight-calculate/',
...
function create_shortcode_tool_ideal_weight($args, $content) {
	...
	wp_enqueue_script( 'ideal-weight-js', get_template_directory_uri() . '/shortcode/calorie/assets/js/ideal-weight-tool.js','','1.1.8');
...
function get_ideal_weight_tool() {
		...
		?>
			...
			<div class="result">
                <div class="item">
                    <p class="title" style="font-weight:600; background:#336699;border: 1px solid #114477;color: #fff;">Formula</p>
                    <p class="value" style="font-weight:600; background:#336699;border: 1px solid #114477;color: #fff;">Ideal Weight</p>
                </div>
                <?php foreach($result as $item):?>
                    <div class="item">
                        <p class="title"><?= $item->title ?></p>
                        <p class="value"><?= $item->pounds . " lbs" ?></p>
                    </div>
                <?php endforeach;?>
            </div>
		<?php
